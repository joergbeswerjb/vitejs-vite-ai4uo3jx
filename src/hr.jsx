...
const AIInsight = ({ candidate }) => {
  ...
  if (!done && !loading) return (
    <button onClick={generate} style={{ padding: "10px 20px", background: BRAND.blue, color: "#fff", border: "none", borderRadius: 4, fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
      Сгенерировать AI-анализ
    </button>
  );
  if (loading) return <div style={{ fontSize: 13, color: "#888", padding: "12px 0" }}>Анализирую профиль кандидата...</div>;
...
export default function App() {
  ...
  const s = {
    wrap: { padding: "1.25rem", maxWidth: 980, margin: "0 auto", fontFamily: "Arial, sans-serif", fontSize: 14 },
    h1: { fontSize: 20, fontWeight: 700, color: BRAND.blue, margin: 0 },
    h2: { fontSize: 15, fontWeight: 600, color: BRAND.blue, margin: "0 0 12px", paddingBottom: 6, borderBottom: "2px solid " + BRAND.green },
    card: { background: "#fff", border: "1px solid " + BRAND.border, borderRadius: 4, padding: "1rem 1.25rem", marginBottom: 12 },
    row: { display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" },
    btn: function(active) { return { padding: "6px 14px", borderRadius: 4, border: "1px solid " + BRAND.border, background: active ? BRAND.blue : "#fff", color: active ? "#fff" : BRAND.dark, cursor: "pointer", fontSize: 13, fontWeight: active ? 600 : 400 }; },
    select: { padding: "6px 10px", borderRadius: 4, border: "1px solid " + BRAND.border, fontSize: 13, background: "#fff", color: BRAND.dark },
    input: { padding: "6px 12px", borderRadius: 4, border: "1px solid " + BRAND.border, fontSize: 13, background: "#fff", color: BRAND.dark, width: 180 },
    stat: { textAlign: "center", padding: "12px 20px", background: BRAND.gray, borderRadius: 4, flex: 1, minWidth: 100 },
  };
  ...
  if (!auth) return (
    <div style={{ fontFamily: "Arial, sans-serif", minHeight: "100vh", background: BRAND.gray }}>
      <div style={{ background: BRAND.blue, padding: "14px 24px", marginBottom: 0 }}>
        <img src="/stadler-logo.png" alt="Stadler" style={{ height: 22, filter: "brightness(0) invert(1)" }} />
      </div>
      <div style={{ maxWidth: 360, margin: "4rem auto", padding: "0 1rem" }}>
        <div style={s.card}>
          ...
          <input type="password" value={pass}
            ...
            placeholder="Пароль..."
            style={{ ...s.input, width: "100%", boxSizing: "border-box", marginBottom: 8, padding: "10px 14px", fontSize: 15, border: passErr ? "1.5px solid #c0392b" : "1px solid " + BRAND.border }} />
      ...
      <div style={s.wrap}>
        <div style={{ ...s.row, justifyContent: "space-between", marginBottom: 16 }}>
          ...
          <div style={s.row}>
            ...
            <button style={{ ...s.btn(false), background: BRAND.gray }} onClick={loadData}>↻ Обновить</button>
          </div>
        </div>
        ...
        {view === "card" && !loading && !selected && (
          <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
            {filtered.map((c, i) => {
              ...
              const rankColor = RANK_COLOR[c["Ранг"]] || "#888";
              return (
                <div key={i} style={{ ...s.card, width: 240, cursor: "pointer", borderTop: "3px solid " + BRAND.blue }} onClick={() => setSelected(c)}>
        ...
        {selected && (
          <div>
            <div style={{ ...s.row, justifyContent: "space-between", marginBottom: 16 }}>
              <button style={s.btn(false)} onClick={() => setSelected(null)}>← Назад</button>
              <button style={{ ...s.btn(false), color: "#c0392b", borderColor: "#c0392b", fontSize: 13 }} onClick={() => deleteCandidate(selected)}>🗑 Удалить запись</button>
            </div>
            <div style={s.card}>
            ...
            <div style={{ ...s.card, background: BRAND.gray, borderLeft: "4px solid " + BRAND.blue }}>
          ...
          <div>
            ...
            <div style={{ display: "flex", gap: 12 }}>
              {compare.map((c, ci) => {
                ...
                const rankColor = RANK_COLOR[c["Ранг"]] || "#888";
                return (
                  <div key={ci} style={{ ...s.card, flex: 1, borderTop: "3px solid " + BRAND.blue }}>
