...
const RAW_QUESTIONS = [
  ...
  { type: "situational", q: "Лучший сотрудник просит повышение которое вы не можете одобрить сейчас. Ваши действия:", options: ["Скажу просто «нет»", "Пообещаю чтобы удержать", "Объясню причины, дам план и назову срок", "Переведу разговор на другую тему"], ans: 2 },
...
// DISC — 4 варианта, вынужденный выбор, все звучат нейтрально
const DISC_QUESTIONS = [
  ...
  { q: "В конфликтной ситуации внутри команды вы:", options: [{ text: "Обозначаю позицию чётко и жду решения", d: "D" }, { text: "Стараюсь снизить напряжение и сохранить отношения", d: "S" }, { text: "Анализирую кто прав по фактам", d: "C" }, { text: "Выслушиваю всех и ищу компромисс который устроит всех", d: "I" }] },
  ...
  { q: "Если бы вас попросили описать себя одним словом коллеги, это скорее всего было бы:", options: [{ text: "Результативный", d: "D" }, { text: "Надёжный", d: "S" }, { text: "Точный", d: "C" }, { text: "Энергичный", d: "I" }] },
];

// Белбин — 9 ролей, 18 вопросов, вынужденный выбор из 4
const BELBIN_ROLES = {
  PL: { name: "Генератор идей", desc: "Креативный, нестандартно мыслит, генерирует оригинальные решения. Лучший в роли источника инноваций.", team: "Незаменим на этапе поиска новых подходов." },
  RI: { name: "Исследователь ресурсов", desc: "Коммуникабельный, легко находит внешние ресурсы и контакты. Умеет использовать возможности.", team: "Силён в нетворкинге и поиске партнёров." },
  CO: { name: "Координатор", desc: "Умеет организовать работу группы и направить людей к цели. Зрелый, уверенный.", team: "Лучший в роли председателя или координатора команды." },
  SH: { name: "Мотиватор", desc: "Динамичный, создаёт давление, не боится препятствий. Добивается результата любой ценой.", team: "Эффективен когда команде нужен толчок." },
  ME: { name: "Аналитик-стратег", desc: "Взвешенный, аналитичный, видит общую картину. Трезво оценивает варианты.", team: "Незаменим при принятии стратегических решений." },
  TW: { name: "Командный игрок", desc: "Дипломатичный, чуткий, поддерживает атмосферу. Сглаживает конфликты.", team: "Ключевой для поддержания здоровья команды." },
  ...
  SP: { name: "Специалист", desc: "Глубокая экспертиза в узкой области. Целеустремлённый профессионал.", team: "Ценен когда нужна конкретная узкая компетенция." },
};
...
const BELBIN_QUESTIONS = [
  ...
  { q: "Мой вклад в командный проект обычно состоит в том что я:", options: [{ text: "Нахожу логические слабости в плане до старта", r: "ME" }, { text: "Слежу чтобы никто не чувствовал себя лишним", r: "TW" }, { text: "Перевожу план в конкретный список задач и делаю их", r: "IMP" }, { text: "Проверяю финальный результат на ошибки и несоответствия", r: "CF" }] },
  ...
  { q: "Когда дедлайн горит я:", options: [{ text: "Мобилизуюсь и давлю на команду чтобы успеть", r: "SH" }, { text: "Спокойно расставляю приоритеты и делаю самое важное", r: "IMP" }, { text: "Поддерживаю команду морально — не паниковать", r: "TW" }, { text: "Перепроверяю что ничего критического не пропущено", r: "CF" }] },
  ...
  { q: "В долгосрочном проекте моя роль обычно:", options: [{ text: "Генерировать новые направления когда проект теряет импульс", r: "PL" }, { text: "Поддерживать связи с внешними партнёрами и ресурсами", r: "RI" }, { text: "Следить за тем чтобы процессы работали как часы", r: "IMP" }, { text: "Финальная проверка перед каждым ключевым этапом", r: "CF" }] },
  ...
  { q: "При запуске нового проекта я лучше всего справляюсь с:", options: [{ text: "Генерацией концепции и видения", r: "PL" }, { text: "Поиском внешних ресурсов партнёров и экспертов", r: "RI" }, { text: "Разработкой структуры и плана выполнения", r: "IMP" }, { text: "Созданием стандартов качества и критериев успеха", r: "CF" }] },
  { q: "Когда другие теряют мотивацию я:", options: [{ text: "Предлагаю новый угол зрения который вдохновляет", r: "PL" }, { text: "Подключаю внешние ресурсы и контакты чтобы сдвинуть с места", r: "RI" }, { text: "Давлю на результат — нужно двигаться вперёд", r: "SH" }, { text: "Поддерживаю лично каждого члена команды", r: "TW" }] },
...
const DISC_PROFILES = {
  D: { name: "Доминирование (D)", color: "#c0392b", desc: "Ориентирован на результат, решителен, берёт инициативу. Работает эффективно под давлением. Может быть жёстким в коммуникации.", team: "Лучший в роли лидера задачи или антикризисного менеджера." },
  I: { name: "Влияние (I)", color: "#e67e22", desc: "Коммуникабелен, умеет вдохновлять и вовлекать людей. Строит отношения легко и быстро.", team: "Силён в продажах, переговорах, командном взаимодействии." },
  S: { name: "Стабильность (S)", color: "#27ae60", desc: "Надёжен, терпелив, лоялен. Ценит предсказуемость и гармонию в команде.", team: "Незаменим как опора команды и хранитель процессов." },
  C: { name: "Соответствие (C)", color: "#2980b9", desc: "Аналитичен, точен, следует стандартам. Высокое качество работы и внимание к деталям.", team: "Ценен в ролях требующих точности: аналитика, контроль, методология." },
};
...
export default function App() {
  ...
  const calcBelbin = () => {
    let counts = {};
    Object.keys(BELBIN_ROLES).forEach(r => counts[r] = 0);
    Object.values(belbinAnswers).forEach(r => { if (counts[r] !== undefined) counts[r]++; });
    const sorted = Object.entries(counts).sort((a, b) => b[1] - a[1]);
    return { counts, primary: sorted[0][0], secondary: sorted[1][0] };
  };

  const getRank = (pct) => {
    if (pct >= 85) return { label: "A — Высокий потенциал", color: "#27ae60", desc: "Исключительный результат. Кандидат демонстрирует сильное аналитическое мышление, быструю обработку информации и высокое качество решений под давлением." };
    if (pct >= 65) return { label: "B — Выше среднего", color: "#2980b9", desc: "Хороший результат. Кандидат справляется со сложными задачами, мыслит системно. Рекомендуется для позиций требующих самостоятельности." };
    if (pct >= 45) return { label: "C — Средний уровень", color: "#e67e22", desc: "Базовый уровень. Кандидат справляется со стандартными задачами. Рекомендуется для структурированных ролей с чётким руководством." };
    return { label: "D — Ниже ожиданий", color: "#c0392b", desc: "Результат ниже минимального порога. Рекомендуется дополнительное собеседование для уточнения причин." };
  };

  // Таймер для скоростных вопросов и DISC
  useEffect(() => {
    if (screen === "iq") {
    ...
    if (screen === "disc") {
      if (!(currentQ in discAnswers)) {
        setTimeLeft(DISC_TIME);
        timerRef.current = setInterval(() => {
          setTimeLeft(t => {
            if (t <= 1) {
              clearInterval(timerRef.current);
              // При истечении времени в DISC — случайный выбор чтобы не пропускать
              const opts = DISC_Q[currentQ].options;
              const rand = opts[Math.floor(Math.random() * opts.length)];
              setDiscAnswers(prev => ({ ...prev, [currentQ]: rand.d }));
  ...
  useEffect(() => {
    if (screen === "done" && !sentRef.current) {
      ...
      const { primary: dp, secondary: ds, counts: dc } = calcDISC();
  ...
  const goNext = (mode) => {
    ...
    if (mode === "iq") {
      ...
      else { setCurrentQ(0); setScreen("belbin"); }
    } else {
      if (currentQ < totalBelbin - 1) setCurrentQ(q => q + 1);
      else setScreen("done");
    }
  };
  ...
  const s = {
    ...
    h2: { fontSize: 17, fontWeight: 600, color: "#111", margin: "1.5rem 0 0.75rem" },
    muted: { fontSize: 14, color: "#666", lineHeight: 1.7 },
    small: { fontSize: 13, color: "#888", lineHeight: 1.6 },
    ...
    tag: (col) => ({ display: "inline-block", fontSize: 12, padding: "3px 10px", borderRadius: 20, background: col + "22", color: col, fontWeight: 600, marginBottom: 6 }),
  };
  ...
  if (screen === "intro") return (
    <div style={s.wrap}>
      ...
      <p style={s.muted}>
        <b>Блок 1 — Логика и мышление</b> (43 вопроса, ~25 мин)<br/>
        Задачи на числовую логику, вербальное мышление, ситуативные и нестандартные задачи. Часть вопросов имеет ограничение по времени.<br/><br/>
        <b>Блок 2 — Стиль работы</b> (20 вопросов, ~10 мин)<br/>
        Вопросы о том как вы обычно действуете в рабочих ситуациях.<br/><br/>
        <b>Блок 3 — Командная роль</b> (18 вопросов, ~10 мин)<br/>
        Вопросы о вашей роли в командной работе.
      </p>
      <div style={s.rule} />
      <h2 style={{ ...s.h2, marginTop: 0 }}>Важно знать</h2>
      <p style={s.muted}>
        • В блоках 2 и 3 нет правильных и неправильных ответов — только честные<br/>
        • Результаты теста видит только HR — вам они не отображаются<br/>
        • Постарайтесь отвечать быстро и интуитивно — первый ответ обычно точнее<br/>
        • Общее время: около 45–50 минут
      </p>
      <div style={s.rule} />
    ...
    return (
      <div style={s.wrap}>
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          <span style={s.small}>Блок 1 · {sectionLabel}</span>
          <span style={s.small}>{currentQ + 1} / {totalIQ}</span>
        </div>
        <div style={s.progress}><div style={s.fill(progPct)} /></div>
        {isSpeed && timeLeft !== null && !answered && (
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 10, color: timeLeft <= 4 ? "#c0392b" : "#e67e22" }}>⏱ {timeLeft} сек</div>
        )}
        {to && <div style={{ fontSize: 13, color: "#c0392b", marginBottom: 10 }}>⏱ Время вышло</div>}
        <div style={{ ...s.card, marginBottom: "1.25rem" }}>
          <p style={{ fontSize: 15, lineHeight: 1.7, margin: 0 }}>{q.q}</p>
        </div>
        {q.options.map((opt, i) => {
        ...
        {(answered || to) && (
          <button style={s.btnPrimary} onClick={() => goNext("iq")}>
            {currentQ < totalIQ - 1 ? "Следующий →" : "Перейти к блоку 2 →"}
          </button>
        )}
      </div>
    );
  ...
  if (screen === "disc") {
    ...
    return (
      <div style={s.wrap}>
        ...
        <div style={s.progress}><div style={s.fill(progPct, "#e67e22")} /></div>
        {timeLeft !== null && !answered && (
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 10, color: timeLeft <= 5 ? "#c0392b" : "#e67e22" }}>⏱ {timeLeft} сек</div>
        )}
        <p style={{ ...s.small, marginBottom: 6 }}>Выберите вариант который <b>точнее всего</b> описывает вас:</p>
        <div style={{ ...s.card, marginBottom: "1.25rem" }}>
          <p style={{ fontSize: 15, lineHeight: 1.7, margin: 0 }}>{q.q}</p>
        </div>
        {q.options.map((opt, i) => {
        ...
        {answered && (
          <button style={s.btnPrimary} onClick={() => goNext("disc")}>
            {currentQ < totalDISC - 1 ? "Следующий →" : "Перейти к блоку 3 →"}
          </button>
        )}
      </div>
    );
  ...
  if (screen === "belbin") {
    ...
    const progPct = Math.round(((currentQ + 1) / totalBelbin) * 100);
    return (
      <div style={s.wrap}>
        <div style={{ display: "flex", justifyContent: "space-between" }}>
        ...
        <div style={s.progress}><div style={s.fill(progPct, "#2980b9")} /></div>
        <p style={{ ...s.small, marginBottom: 6 }}>Выберите вариант который <b>точнее всего</b> описывает вас в команде:</p>
        <div style={{ ...s.card, marginBottom: "1.25rem" }}>
          <p style={{ fontSize: 15, lineHeight: 1.7, margin: 0 }}>{q.q}</p>
        </div>
        {q.options.map((opt, i) => {
        ...
        {answered && (
          <button style={s.btnPrimary} onClick={() => goNext("belbin")}>
            {currentQ < totalBelbin - 1 ? "Следующий →" : "Завершить тест →"}
          </button>
        )}
      </div>
    );
    ...
    return (
      <div style={{ ...s.wrap, textAlign: "center", paddingTop: "4rem" }}>
        ...
        <div style={{ marginTop: 32 }}>
          <button style={{ ...s.btnPrimary, background: "#f0f0f0", color: "#333" }} onClick={resetAll}>← Пройти снова</button>
        </div>
      </div>
    );
  }
